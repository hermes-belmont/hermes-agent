...
import { CheckCircle2, Clock, Edit3, Plus, Target, Trash2 } from "lucide-react";
...
export function TrackingView({ agents, initialAgentId, initialItemId }: Props) {
  ...
  const load = useCallback(async () => { setLoading(true); try { const raw = await api.listTrackedItems(); setItems(raw.map((it) => ({ ...it, tags: Array.isArray(it.tags) ? it.tags : [] }))); } finally { setLoading(false); } }, []);
  ...
  return <section className="space-y-5">
    <div className="flex flex-wrap items-end justify-between gap-3">
      <div><p className="text-xs uppercase tracking-[0.24em] text-[var(--warm-glow)]">Kanban</p><h1 className="mt-2 font-expanded text-3xl uppercase tracking-[0.08em]">Kanban</h1><p className="mt-2 text-sm text-muted-foreground">Watch items and native task boards</p></div>
      {activeTab === "watch" ? <Button onClick={openAdd}><Plus className="mr-2 h-4 w-4" /> Add Item</Button> : null}
    </div>

    <div className="inline-flex rounded-2xl border border-foreground/10 bg-foreground/[0.03] p-1" role="tablist" aria-label="Kanban sections">
      {([{ id: "watch", label: "Watch Items" }, { id: "board", label: "Board" }] as Array<{ id: Tab; label: string }>).map((tab) => <button key={tab.id} type="button" role="tab" aria-selected={activeTab === tab.id} onClick={() => setActiveTab(tab.id)} className={cn("rounded-xl px-4 py-2 text-xs uppercase tracking-[0.16em] transition", activeTab === tab.id ? "bg-[var(--warm-glow)]/15 text-[var(--warm-glow)]" : "text-muted-foreground hover:text-foreground")}>{tab.label}</button>)}
    </div>

    {activeTab === "board" ? <KanbanBoardPanel /> : <>
      ...
      {loading ? <Card><CardContent className="py-10 text-muted-foreground">Loading tracked items...</CardContent></Card> : filtered.length === 0 ? <Card className="border-dashed"><CardContent className="flex flex-col items-center justify-center py-16 text-center"><Target className="h-10 w-10 text-muted-foreground" /><h2 className="mt-4 font-expanded text-xl uppercase">No items match these filters</h2><Button className="mt-4" onClick={openAdd}>Add Item</Button></CardContent></Card> : <div className="space-y-3">{filtered.map((item) => <Card key={item.id} id={`tracked-${item.id}`} className={cn(initialItemId === item.id && "ring-2 ring-[var(--warm-glow)]")}><CardContent className="pt-5"><div className="flex flex-wrap justify-between gap-4"><div className="min-w-0 flex-1"><h2 className="text-lg font-semibold">{item.title}</h2>{item.description && <p className="mt-1 line-clamp-2 text-sm text-muted-foreground">{item.description}</p>}<div className="mt-3 flex flex-wrap gap-2"><Pill>{agentMap.get(item.agent_id)?.name ?? item.agent_id}</Pill><Pill>{titleCase(item.entity)}</Pill><Pill>{titleCase(item.category)}</Pill><Pill tone={item.priority}>{item.priority}</Pill><Pill tone={item.status === "active" ? "active" : item.status === "done" ? "done" : "neutral"}>{item.status}</Pill><Pill>{relDate(item)}</Pill></div>{item.tags.length > 0 && <div className="mt-2 flex flex-wrap gap-1">{item.tags.map((tag) => <span key={tag} className="rounded-md bg-foreground/[0.05] px-2 py-0.5 text-xs text-muted-foreground">#{tag}</span>)}</div>}</div><div className="flex flex-wrap items-start justify-end gap-2"><Button size="sm" variant="ghost" onClick={() => openEdit(item)}><Edit3 className="mr-1 h-3.5 w-3.5" />Edit</Button>{item.status !== "snoozed" && <Button size="sm" variant="ghost" onClick={() => act(item, "snooze")}><Clock className="mr-1 h-3.5 w-3.5" />Snooze</Button>}<Button size="sm" variant="ghost" onClick={() => act(item, "done")}><CheckCircle2 className="mr-1 h-3.5 w-3.5" />Mark Done</Button>{item.status !== "active" && <Button size="sm" variant="ghost" onClick={() => act(item, "reactivate")}>Reactivate</Button>}<Button size="sm" variant="ghost" onClick={() => remove(item)}><Trash2 className="mr-1 h-3.5 w-3.5" />Delete</Button></div></div></CardContent></Card>)}</div>}
    </>}

    {modal && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm" onMouseDown={(e) => { if (e.target === e.currentTarget) setModal(null); }}><Card className="max-h-[90vh] w-full max-w-2xl overflow-y-auto"><CardHeader><CardTitle className="font-expanded uppercase tracking-[0.08em]">{modal.mode === "add" ? "Add Item" : "Edit Item"}</CardTitle><CardDescription>Structured watch item for daily briefing context injection.</CardDescription></CardHeader><CardContent className="grid gap-3 md:grid-cols-2"><label className="text-sm">Agent<select className="mt-1 w-full rounded-lg border border-foreground/10 bg-background px-3 py-2" value={draft.agent_id} onChange={(e) => { const agent = agentMap.get(e.target.value); setAgentValidation(""); setDraft({ ...draft, agent_id: e.target.value, entity: draft.entity ?? entityForAgent(agent) }); }}><option value="">Select agent...</option>{agents.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</select>{agentValidation && <p className="mt-1 text-xs text-red-200">{agentValidation}</p>}</label><label className="text-sm md:col-span-2">Title<Input className="mt-1" value={draft.title} onChange={(e) => setDraft({ ...draft, title: e.target.value })} /></label><label className="text-sm md:col-span-2">Description<textarea className="mt-1 min-h-24 w-full rounded-lg border border-foreground/10 bg-background px-3 py-2" value={draft.description ?? ""} onChange={(e) => setDraft({ ...draft, description: e.target.value })} /></label><label className="text-sm">Entity<select className="mt-1 w-full rounded-lg border border-foreground/10 bg-background px-3 py-2" value={draft.entity ?? ""} onChange={(e) => setDraft({ ...draft, entity: (e.target.value || null) as TrackedItem["entity"] })}><option value="">None</option>{entities.map((e) => <option key={e} value={e}>{titleCase(e)}</option>)}</select></label><label className="text-sm">Category<select className="mt-1 w-full rounded-lg border border-foreground/10 bg-background px-3 py-2" value={draft.category ?? "watch"} onChange={(e) => setDraft({ ...draft, category: e.target.value as TrackedItem["category"] })}>{categories.map((c) => <option key={c} value={c}>{titleCase(c)}</option>)}</select></label><label className="text-sm">Priority<select className="mt-1 w-full rounded-lg border border-foreground/10 bg-background px-3 py-2" value={draft.priority ?? "medium"} onChange={(e) => setDraft({ ...draft, priority: e.target.value as TrackedItem["priority"] })}>{priorities.map((p) => <option key={p} value={p}>{titleCase(p)}</option>)}</select></label><label className="text-sm">Status<select className="mt-1 w-full rounded-lg border border-foreground/10 bg-background px-3 py-2" value={draft.status ?? "active"} onChange={(e) => setDraft({ ...draft, status: e.target.value as TrackedItemStatus })}>{statuses.map((s) => <option key={s} value={s}>{titleCase(s)}</option>)}</select></label><label className="text-sm">Due date<Input type="date" className="mt-1" value={draft.due_date ?? ""} onChange={(e) => setDraft({ ...draft, due_date: e.target.value })} /></label><label className="text-sm">Recurrence<select className="mt-1 w-full rounded-lg border border-foreground/10 bg-background px-3 py-2" value={draft.recurrence ?? ""} onChange={(e) => setDraft({ ...draft, recurrence: (e.target.value || null) as TrackedItem["recurrence"] })}>{recurrences.map((r) => <option key={r || "none"} value={r}>{r ? titleCase(r) : "None"}</option>)}</select></label><label className="text-sm md:col-span-2">Tags<Input className="mt-1" value={draft.tagsText ?? ""} onChange={(e) => setDraft({ ...draft, tagsText: e.target.value })} placeholder="comma, separated" /></label><label className="text-sm md:col-span-2">Notes<textarea className="mt-1 min-h-20 w-full rounded-lg border border-foreground/10 bg-background px-3 py-2" value={draft.notes ?? ""} onChange={(e) => setDraft({ ...draft, notes: e.target.value })} /></label><div className="flex justify-end gap-2 md:col-span-2"><Button variant="outline" onClick={() => setModal(null)}>Cancel</Button><Button onClick={save}>Save</Button></div></CardContent></Card></div>}
  </section>;
}
