...
export default function ConfigPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([api.getConfig(), api.getSchema(), api.getDefaults()])
      .then(([configResp, schemaResp, defaultsResp]) => {
        if (cancelled) return;
      ...
      .catch((err) => {
        ...
        setLoadError(detail);
        showToast(`Failed to load config dashboard: ${detail}`, "error");
      });
    return () => {
      cancelled = true;
    };
  ...
  const handleYamlSave = async () => {
    ...
    try {
      ...
      try {
        ...
        const detail = err instanceof Error ? err.message : String(err);
        setLoadError(detail);
        showToast(`Saved raw YAML but failed to refresh form state: ${detail}`, "error");
      }
    } catch (e) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {loadError && (
        <div className="border border-warning/30 bg-warning/[0.06] px-3 py-2 text-sm text-warning">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="font-medium">Config dashboard data may be stale</p>
              <p className="text-xs text-warning/80 mt-0.5 break-all">{loadError}</p>
            </div>
            <button
              type="button"
