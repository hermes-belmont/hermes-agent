...
export default function SessionsPage() {
  ...
  const searchResultSessions: SessionInfo[] = searchResults
    ? searchResults.map((result) => ({
        id: result.session_id,
        source: result.source ?? null,
        model: result.model ?? null,
        title: result.title ?? result.preview ?? null,
        started_at: result.session_started ?? 0,
        ended_at: result.is_active ? null : result.last_active ?? result.session_started ?? null,
        last_active: result.last_active ?? result.session_started ?? 0,
  ...
  const displayedSessions = searchResults ? searchResultSessions : sessions;
