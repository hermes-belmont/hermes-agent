...
class TestWebServerEndpoints:
    ...
    def test_gateway_restart_endpoint_invokes_cli(self, monkeypatch):
        ...
        monkeypatch.setattr(web_server.subprocess, "run", _fake_run)

        resp = self.client.post("/api/gateway/restart")
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_gateway_restart_endpoint_surfaces_failures(self, monkeypatch):
        import hermes_cli.web_server as web_server
        ...
        assert "restart failed" in resp.json()["detail"]
...
class TestNewEndpoints:
    ...
    def test_status_summary_returns_lightweight_recent_and_active_sessions(self):
        from hermes_state import SessionDB

        db = SessionDB()
        try:
            active_id = db.create_session("active-dashboard-test", "cli", model="openai/gpt-test")
            db.set_session_title(active_id, "Active dashboard test")
            db.append_message(active_id, "user", "hello from active")

            recent_id = db.create_session("recent-dashboard-test", "discord", model="openai/gpt-test")
            db.set_session_title(recent_id, "Recent dashboard test")
            db.append_message(recent_id, "user", "hello from recent")
            db.end_session(recent_id, "completed")
        finally:
            db.close()

        resp = self.client.get("/api/status/summary")
        assert resp.status_code == 200
        data = resp.json()
        assert "recent_sessions" in data
        assert "active_session_details" in data
        assert "current_config_version" in data
        assert "latest_config_version" in data
        assert any(s["id"] == active_id for s in data["active_session_details"])
        assert any(s["id"] == recent_id for s in data["recent_sessions"])
        assert all("preview" in s for s in data["recent_sessions"])
        assert all("message_count" in s for s in data["recent_sessions"])

    def test_search_sessions_returns_rich_session_metadata(self):
        from hermes_state import SessionDB

        db = SessionDB()
        try:
            session_id = db.create_session(
                "search-metadata-session",
                "cli",
                model="openai/gpt-test",
            )
            db.set_session_title(session_id, "Search Metadata Session")
            db.append_message(session_id, "user", "optimize dashboard search metadata")
            db.append_message(session_id, "assistant", "done")
        finally:
            db.close()

        resp = self.client.get("/api/sessions/search?q=optimize")
        assert resp.status_code == 200
        results = resp.json()["results"]
        match = next(r for r in results if r["session_id"] == session_id)
        assert match["title"] == "Search Metadata Session"
        assert match["message_count"] >= 2
        assert "last_active" in match
        assert "preview" in match

    def test_chat_session_metadata_crud(self):
        from hermes_state import SessionDB

        db = SessionDB()
        try:
            session_id = db.create_session(
                "chat-metadata-session",
                "cli",
                model="openai/gpt-test",
            )
            db.set_session_title(session_id, "Chat metadata session")
        finally:
            db.close()

        put_resp = self.client.put(
            f"/api/chat/session-metadata/{session_id}",
            json={"pinned": True, "custom_title": "Pinned title"},
        )
        assert put_resp.status_code == 200
        assert put_resp.json()["meta"]["pinned"] is True
        assert put_resp.json()["meta"]["custom_title"] == "Pinned title"

        get_resp = self.client.get("/api/chat/session-metadata")
        assert get_resp.status_code == 200
        assert get_resp.json()["items"][session_id]["custom_title"] == "Pinned title"

        delete_resp = self.client.delete(f"/api/chat/session-metadata/{session_id}")
        assert delete_resp.status_code == 200
        get_after = self.client.get("/api/chat/session-metadata")
        assert session_id not in get_after.json()["items"]
