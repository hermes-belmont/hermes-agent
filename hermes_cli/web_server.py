...
_SKILLS_CACHE_TTL_SECONDS = 10.0
_skills_cache_lock = threading.Lock()
_skills_cache_payload: list[dict[str, Any]] | None = None
_skills_cache_expires_at = 0.0
...
async def get_status_summary():
    ...
    try:
        payload.update(_build_status_session_summary())
    except Exception:
        _log.exception("GET /api/status/summary failed while loading session summary")
        payload.setdefault("recent_sessions", [])
        payload.setdefault("active_session_details", [])
    return payload
...
async def _build_status_payload() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            row = cur.fetchone()
            active_sessions = int(row["active_count"] if row and row["active_count"] is not None else 0)
        finally:
            db.close()
    except Exception:
        pass
    ...
    version_relation = (
        "outdated" if current_ver < latest_ver
        else "ahead" if current_ver > latest_ver
        else "current"
    )
    ...
    return {
        ...
        "current_config_version": current_ver,
        "latest_config_version": latest_ver,
        "config_version_status": version_relation,
        "config_is_outdated": current_ver < latest_ver,
...
def _build_status_session_summary(limit: int = 8) -> Dict[str, Any]:
    from hermes_state import SessionDB

    db = SessionDB()
    try:
        sessions = db.list_sessions_rich(limit=max(limit * 4, 24))
        now = time.time()
        for s in sessions:
            s["is_active"] = _session_is_active(s, now=now)

        active = [s for s in sessions if s["is_active"]][:limit]
        recent = [s for s in sessions if not s["is_active"]][:limit]
        return {
...
def _run_gateway_command(action: str) -> dict[str, Any]:
    ...
    if result.returncode != 0:
        raise HTTPException(
            status_code=500,
            detail=error or output or f"Gateway {action} failed",
        )
    return {
        "ok": True,
        "action": action,
...
async def restart_gateway():
    return _run_gateway_command("restart")
    ...
    try:
        ...
        try:
            ...
            for m in matches:
                ...
                if sid in seen:
                    continue
                session_row = db._get_session_rich_row(sid) or {}
                seen[sid] = {
                    "session_id": sid,
                    "snippet": m.get("snippet", ""),
                    "role": m.get("role"),
                    "source": session_row.get("source", m.get("source")),
                    "model": session_row.get("model", m.get("model")),
                    "session_started": session_row.get("started_at", m.get("session_started")),
                    "title": session_row.get("title"),
                    "preview": session_row.get("preview", ""),
                    "message_count": session_row.get("message_count", 0),
                    "tool_call_count": session_row.get("tool_call_count", 0),
                    "last_active": session_row.get("last_active", m.get("session_started")),
                    "is_active": _session_is_active(session_row, now=now) if session_row else False,
                }
...
async def delete_session_endpoint(session_id: str):
    from hermes_state import SessionDB
    from hermes_cli.chat_api import delete_session_metadata_entry
    db = SessionDB()
    try:
...
def _invalidate_skills_cache() -> None:
    global _skills_cache_payload, _skills_cache_expires_at
    with _skills_cache_lock:
        _skills_cache_payload = None
        _skills_cache_expires_at = 0.0


def _get_cached_skills_payload() -> list[dict[str, Any]]:
    global _skills_cache_payload, _skills_cache_expires_at
    now = time.time()
    with _skills_cache_lock:
        if _skills_cache_payload is not None and now < _skills_cache_expires_at:
            return [dict(item) for item in _skills_cache_payload]
    ...
    with _skills_cache_lock:
        _skills_cache_payload = [dict(item) for item in skills]
        _skills_cache_expires_at = now + _SKILLS_CACHE_TTL_SECONDS
        return [dict(item) for item in _skills_cache_payload]


@app.get("/api/skills")
async def get_skills():
    return _get_cached_skills_payload()
...
try:
    from hermes_cli.chat_api import router as chat_api_router
    app.include_router(chat_api_router)
except Exception as exc:
    _log.warning("Failed to mount chat API routes: %s", exc)
